...
def tour(friends, friend_towns, home_to_town_distances):
    ...
    for town in dict_friend_towns.copy(): # Удаление лишних городов
        if town not in friends:
            del dict_friend_towns[town]

    for friend in friends: # Удаление лишних друзей
    ...
    # Добавляем к результату расстояния от последнего друга до дома
